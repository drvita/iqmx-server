...
export const organization = pgTable(
  ...
  (t) => [index("organization_external_customer_idx").on(t.externalCustomerId)]
);
...
export const conversation = pgTable(
  ...
  (t) => [
    // Una conversación real por contacto y por número telefónico; las de prueba no compiten.
    uniqueIndex("conversation_org_contact_phone_real_uq")
      .on(t.organizationId, t.contactId, sql`coalesce(${t.phoneNumberId}, '')`)
...
export const metaCredentials = pgTable(
  "meta_credentials",
  {
    ...
    status: text("status", { enum: ["connected", "reconnect_required"] })
      .notNull()
      .default("connected"),
    /** Etiqueta o nombre identificador de la línea (ej. "Ventas", "Soporte"). */
    label: text("label"),
    /** Si es la línea predeterminada de la organización para envíos directos. */
    isDefault: boolean("is_default").notNull().default(false),
    /** Si el asistente IA atiende en esta línea telefónica. */
    aiEnabled: boolean("ai_enabled").notNull().default(true),
    /** Asistente IA conversacional asignado a esta línea (siempre conversacional, no tool). */
    assistantId: text("assistant_id").references(() => agentProfile.id, {
      onDelete: "set null",
    }),
    /** Método por el cual se conectó este número. */
  ...
  (t) => [
    index("meta_credentials_org_idx").on(t.organizationId),
...
export const agentProfile = pgTable(
  "agent_profile",
  {
    ...
    organizationId: text("organization_id")
      .notNull()
      .references(() => organization.id, { onDelete: "cascade" }),
    /** conversational = atiende clientes en WhatsApp · tool = procesos internos y análisis */
    type: text("type", { enum: ["conversational", "tool"] })
      .notNull()
      .default("conversational"),
    enabled: boolean("enabled").notNull().default(false),
    /** Si es el asistente predeterminado de la organización. */
    isDefault: boolean("is_default").notNull().default(false),
...
/* ============================================================
 * Multi-número & Permisos Dinámicos (RBAC)
 * ============================================================ */

/** Asignación de líneas telefónicas a miembros/operadores humanos. */
export const memberPhoneAccess = pgTable(
  "member_phone_access",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id")
  ...
  (t) => [
    uniqueIndex("member_phone_access_uq").on(
      t.organizationId,
      t.memberId,
      t.phoneNumberId
    ),
    index("member_phone_access_org_idx").on(t.organizationId, t.memberId),
  ]
);

/** Matriz de permisos dinámicos por rol para módulos del CRM (pipeline, agenda, contacts, etc.). */
export const rolePermission = pgTable(
  ...
  (t) => [
    uniqueIndex("role_perm_org_role_mod_uq").on(
      t.organizationId,
      t.role,
      t.module
    ),
    index("role_perm_org_idx").on(t.organizationId, t.role),
  ]
);

/** Configuración SaaS multi-tenant, límites de membresía y secretos operativos de la organización. */
export const organizationSettings = pgTable(
  "organization_settings",
  {
    ...
    // --- Límites y Características de Membresía (Control Central M2M) ---
    ...
    channels: text("channels").notNull().default("whatsapp"),
    maxWhatsappAccounts: integer("max_whatsapp_accounts").notNull().default(1),
    maxTeamMembers: integer("max_team_members").notNull().default(2),
    maxContacts: integer("max_contacts").notNull().default(100),
    maxTokensIn: integer("max_tokens_in").notNull().default(50000),
    maxTokensOut: integer("max_tokens_out").notNull().default(20000),
    aiEnabled: boolean("ai_enabled").notNull().default(true),
    labEnabled: boolean("lab_enabled").notNull().default(false),
    tasksEnabled: boolean("tasks_enabled").notNull().default(false),

    // --- Configuración Operativa (Gestionable por el Tenant) ---
  ...
  (t) => [uniqueIndex("org_settings_org_uq").on(t.organizationId)]
);
