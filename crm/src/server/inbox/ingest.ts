...
export async function getOrCreateConversation(
  organizationId: string,
  contactId: string,
  opts?: { channel?: Channel; threadRef?: string | null; phoneNumberId?: string | null }
) {
  return findOrCreateRealConversation(organizationId, contactId, opts);
}

export async function findOrCreateRealConversation(
  organizationId: string,
  contactId: string,
  opts?: { channel?: Channel; threadRef?: string | null; phoneNumberId?: string | null }
) {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        opts?.phoneNumberId
          ? eq(schema.conversation.phoneNumberId, opts.phoneNumberId)
          : undefined
      )
    )
    .limit(1);
  ...
  if (opts?.threadRef && existing.channelThreadRef !== opts.threadRef) {
    updates.channelThreadRef = opts.threadRef;
  }
  if (opts?.phoneNumberId && !existing.phoneNumberId) {
    updates.phoneNumberId = opts.phoneNumberId;
  }
  if (Object.keys(updates).length > 0) {
    ...
    if (updates.channelThreadRef) existing.channelThreadRef = opts?.threadRef ?? null;
    if (updates.phoneNumberId) existing.phoneNumberId = opts?.phoneNumberId ?? null;
  }
  return existing;
}
...
export async function processEchoesValue(value: WebhookValue): Promise<void> {
  ...
  for (const echo of echoes) {
    ...
    try {
      await ingestManualEcho(credentials.organizationId, echo, phoneNumberId);
    } catch (err) {
...
export async function ingestInboundMessage(input: {
  ...
  /** Línea / número de WhatsApp por el que ingresó el mensaje. */
