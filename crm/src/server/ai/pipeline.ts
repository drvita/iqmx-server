...
/** Cancela el turno pendiente en memoria de una conversación (ej. al recibir un echo). */
export function cancelAgentTurn(conversationId: string): void {
  const map = coalesceMap();
  const entry = map.get(conversationId);
  if (entry) {
    if (entry.timer) clearTimeout(entry.timer);
...
export async function runAgentTurn(conversationId: string): Promise<void> {
  ...
  if (conversation.assistantId) {
    ...
    const creds = await db
      .select({
        ...
        assistantId: schema.metaCredentials.assistantId,
      })
      .from(schema.metaCredentials)
      .where(eq(schema.metaCredentials.phoneNumberId, conversation.phoneNumberId))
      .limit(1);
  ...
  if (targetAssistantId) {
    const rows = await db
      ...
      .where(
        and(
          eq(schema.agentProfile.id, targetAssistantId),
