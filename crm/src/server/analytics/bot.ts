...
export async function botBlock(
  ...
  const hasAgenda = agenda !== undefined ? agenda : await isAgendaEnabled(organizationId);
  ...
  const [conteos, primera, handoffs, sesiones, ficha] = await Promise.all([
    ...
    hasAgenda ? sesionesDelPeriodo(organizationId, start, end) : Promise.resolve(null),
