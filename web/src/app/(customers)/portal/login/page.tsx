...
export default function PortalLoginPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const detail = data?.detail || 'Correo electrónico o contraseña incorrectos.';
        setErrorMsg(detail);
        return;
      }
      ...
      // Si tiene intención de compra pendiente, enviarlo con el flag para pagar
      if (pendingIntent) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans text-gray-900">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center text-center">
        ...
        {pendingIntent && (
          <div className="mt-4 rounded-xl bg-blue-50 border border-blue-200 p-3 text-xs text-blue-800 flex items-center justify-center gap-2">
            <ShoppingBagIcon className="h-4 w-4 text-blue-600 shrink-0" />
            <span>
              Tienes pendiente la compra de <strong>{pendingIntent.plan_name}</strong> (${pendingIntent.price_mxn} MXN/mes). Inicia sesión para continuar.
            </span>
          </div>
        )}
      </div>
